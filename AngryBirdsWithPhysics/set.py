...
def draw_object(calc,trail):
    ...
    font = pygame.font.Font('GodoM.ttf',20)
    highest = font.render(f'(R,H) : ({round(highest_point[0])},{round(highest_point[1])})', True, (0,0,0))
...
def calc(t):
    ...
    v = ((v0 * m.cos(theta))**2 + (v0 * m.cos(theta) - 9.8*t)**2)**0.5
    a = 9.8 * t
    angle_radians = m.atan2((v0 * m.sin(m.radians(angle))) - (9.8 * t), v0 * m.cos(m.radians(angle)))
    ...
    #R = v0**2 * m.sin(m.radians(2*angle)) / 9.8
